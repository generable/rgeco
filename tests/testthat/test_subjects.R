...
testthat::test_that('query for subjects data works when providing projectversion id', {
  ...
  s <- fetch_subjects(project_version_id = pv)
  testthat::expect_is(s, 'data.frame')
  expect_no_duplicates(s, by = dplyr::vars(subject_id))
})
