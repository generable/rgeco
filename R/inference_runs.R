...
list_parameter_names <- function(run_id, project = NULL, project_version_id = NULL) {
  ...
  r = run_id
  return(sort(unlist((runs %>% filter(run_id == r) %>% pull(run_quantiles))[[1]]$parameter_names)))
}
...
list_predictive_names <- function(run_id, project = NULL, project_version_id = NULL) {
  ...
  runs <- list_runs(project_version_id = pv_id)
  r = run_id
  return(sort(unlist((runs %>% filter(run_id == r) %>% pull(run_quantiles))[[1]]$predictive_names)))
}